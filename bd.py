...
def date_proverka(d, a):
    ...
    with sqlite3.connect("database.db") as db:
        ...
        flag = True
    return all_data
#функция выводит не тот результат, который вывод и консоль
def srednii(d, a):
    count = 0
    cat = 0
...
def get_most_expens_item(): #самая затратная сфера за весь период
    data = get_statistic_data()
    return max(list(data), key=lambda x:x['amount'])['name']

def alina(): #сумма расходов по этой сфере за весь период
